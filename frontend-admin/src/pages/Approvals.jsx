...
import { Check, X, Download, ExternalLink, Calendar, User, Film, Tv, Loader } from 'lucide-react'
import { approvalsApi, mediaApi } from '../services/api.js'

export default function Approvals() {
  const [items, setItems] = useState({ shows: [], episodes: [] })
  const [loading, setLoading] = useState(false)
  const [activeTab, setActiveTab] = useState('all') // 'all', 'shows', 'episodes'
  ...
  if (loading && !items.shows.length && !items.episodes.length) {
    return (
      <div className="page-enter" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 300, gap: 12 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text3)' }}>Loading pending approvals...</div>
      </div>
    )
  }
    ...
    <div className="page-enter">
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text3)', marginTop: 4 }}>
          Review and approve pending course submissions and new video lectures submitted by teachers.
        </div>
      </div>
                        ...
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                          ...
                          {e.video_source === 'YOUTUBE' ? (
                            ...
                            <button
                              ...
                              style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}
                              title="Download original video file locally to inspect content"
                            >
