        ...
        <ModalSection title={`Episodes (${episodes.length})`}>
          {episodes.length === 0 && !addingEp && (
            <div style={{ textAlign: 'center', padding: '32px 0', color: 'var(--text3)', fontSize: 13 }}>
              No lectures yet. Add your first episode.
            </div>
          )}
          {episodes.map((ep, idx) => (
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      if (val === 'free') {
                        ...
                        if (form.is_free) {
                          alert('Show-only lectures are not allowed on free shows!');
                          return;
                        }
  ...
  return (
    <div className="page-enter">
      ...
      <div className="metrics-grid" style={{ gridTemplateColumns: 'repeat(4,1fr)', marginBottom: 16 }}>
        {[
          { label: 'Total Course', value: dramas.length, sub: `${dramas.filter(d => d.episodes.some(e => !e.is_free)).length} with paid lectures`, color: 'var(--accent2)' },
          { label: 'Total Lectures', value: dramas.reduce((a, d) => a + d.episodes.length, 0), sub: 'across all dramas', color: 'var(--text)' },
                ...
                <div key={d.id}>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: 24, alignItems: 'center', minWidth: 'fit-content', flexShrink: 0 }}>
                      ...
                      <span className={`badge ${d.approval_status === 'PUBLISHED' ? 'badge-green' :
                        ...
                        <div style={{
                          ...
                          {d.episodes.map((ep, idx) => (
                            ...
                            >
                              ...
                              {/* Lecture Status (Transcoding) */}
                              <div style={{ minWidth: 80, flexShrink: 0 }}>
                                <span className={`badge ${ep.status === 'ready' ? 'badge-green' :
                              ...
                              {/* Lecture Approval Status */}
                              <div style={{ minWidth: 100, flexShrink: 0 }}>
                                <span className={`badge ${ep.approval_status === 'PUBLISHED' ? 'badge-green' :
